...
let targetSnapshot = [];
let activitySnapshots = { teams: [], archivedTeams: [], controlPlaneEnabled: false };
...
export function updateActivitySnapshots(update) {
    ...
    if (next.teams === activitySnapshots.teams
        ...
        && next.controlPlaneEnabled === activitySnapshots.controlPlaneEnabled)
        return;
...
export const TEAM_CHAT_URL = '/plugins/dsh-agent-teams/chat';
...
export async function loadTeamChatPage(team, cursor = 0, limit = 100, fetcher = fetch) {
