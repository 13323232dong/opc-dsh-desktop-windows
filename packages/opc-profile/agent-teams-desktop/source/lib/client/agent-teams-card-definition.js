...
export function parseAgentTeamsCreateArgs(value) {
    try {
        ...
        if (name === '')
            return undefined;
        // This must mirror the host's path-safe team key for ordinary names.
        // An ASCII-only expression turns every CJK name into the same "team"
        // fallback, so the browser polls an unrelated activity record.
