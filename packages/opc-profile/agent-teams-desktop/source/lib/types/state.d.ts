 ...
 */
import { type TaskStatus, type TeamChatMessage, type TeamMessage, type TeamState, type TeamTask } from './types.ts';
/** Mailbox key of the captain. */
export declare const CAPTAIN_KEY = "captain";
/** Maximum UTF-16 code units accepted in one plain-text team message. */
export declare const TEAM_CHAT_CONTENT_MAX_LENGTH = 8000;
/** Maximum chat page size exposed by state/HTTP readers. */
export declare const TEAM_CHAT_MAX_PAGE_LIMIT = 100;
/**
...
/** Input accepted by {@link createTeamChatMessage}. */
export type TeamChatMessageInput = Omit<TeamChatMessage, 'id' | 'ts' | 'kind'> & {
    readonly id?: string;
    readonly ts?: number;
    readonly kind?: TeamChatMessage['kind'];
};
/** Create and freeze one strictly validated plain-text chat record. */
export declare function createTeamChatMessage(input: TeamChatMessageInput): TeamChatMessage;
/** Read the append-only team chat, oldest first; malformed records are skipped. */
export declare function readTeamChat(stateRoot: string, teamId: string, onMalformedLine?: (lineNumber: number) => void): Promise<TeamChatMessage[]>;
/** Append exactly the next immutable chat record. Callers serialize with the team lock. */
export declare function appendTeamChat(stateRoot: string, teamId: string, message: TeamChatMessage): Promise<void>;
...
export declare function readTeamChatPage(stateRoot: string, teamId: string, options: {
    readonly cursor: number;
    readonly limit: number;
}): Promise<TeamChatPage>;
/**
