...
if (process.platform === 'win32') {
  // icon.ico is committed from the approved source image. Windows runners do
  // not ship macOS's sips/iconutil utilities, so regenerating it here would
  // turn an otherwise valid package build into a platform-tooling failure.
  await access(icoDestination)
  console.log(`Using committed Windows icon: ${path.relative(projectRoot, icoDestination)}`)
} else {
