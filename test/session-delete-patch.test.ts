...
// Verify that the reproducible patch and installed candidate expose the same deletion contract.
const patchedPackages = [
  {
    name: 'dsh-session-persistence',
    version: '0.1.5-rc.2',
    file: 'lib/index.js',
    markers: ['async delete(id)', 'session deletion is unsupported']
  },
  {
    name: 'dsh-session-persistence-jsonl',
    version: '0.1.5-rc.2',
    file: 'lib/index.js',
    markers: ['async delete(id)', 'this.tracker.claimWrite(id)', 'await rm(selected.sourcePath)']
  },
  {
    name: 'dsh-workspace',
    version: '0.1.5-rc.2',
  ...
  },
  {
    ...
    version: '0.1.5-rc.2',
    file: 'lib/index.js',
    markers: ['disposeOwned(sessionId)', 'persistence.delete(request.sessionId)', 'workspaceRegistry.deleteSession(request.sessionId,']
  },
  {
    name: 'dsh-api-session-controller',
    version: '0.1.5-rc.2',
...
describe('permanent session deletion dependency patches', () => {
  ...
  it('removes one materialized JSONL log without touching another session', async () => {
    ...
    try {
      ...
      await removedHandle.append(event)
