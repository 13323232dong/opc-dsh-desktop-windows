...
describe('OPC desktop profile', () => {
  it('exports an immutable, self-contained plugin manifest', () => {
    ...
    expect(profile.plugins).toContainEqual(expect.objectContaining({
      name: '@opc/dsh-viral-chase', version: '0.1.47', artifact: 'plugins/opc-dsh-viral-chase-0.1.47.tgz'
    }))
    expect(profile.plugins).toContainEqual(expect.objectContaining({
    ...
    expect(profile.plugins).toContainEqual(expect.objectContaining({
      name: '@opc/DSH-ai-customer-service', version: '0.1.26',
      artifact: 'plugins/opc-DSH-ai-customer-service-0.1.26.tgz', client: true
    }))
