...
describe('OPC desktop profile', () => {
  it('exports an immutable, self-contained plugin manifest', () => {
    ...
    expect(profile.plugins).toContainEqual(expect.objectContaining({
      name: '@nanmicoder/dsh-agent-teams', version: '0.1.8-opc-desktop.10', artifact: 'plugins/nanmicoder-dsh-agent-teams-0.1.8-opc-desktop.10.tgz'
    }))
    expect(profile.plugins).toContainEqual(expect.objectContaining({
    ...
    expect(profile.plugins).toContainEqual(expect.objectContaining({
      name: '@opc/dsh-viral-chase', version: '0.1.56', artifact: 'plugins/opc-dsh-viral-chase-0.1.56.tgz'
    }))
    expect(profile.plugins).toContainEqual(expect.objectContaining({
    ...
    expect(profile.plugins).toContainEqual(expect.objectContaining({
      name: '@opc/DSH-ai-customer-service', version: '0.1.30',
      artifact: 'plugins/opc-DSH-ai-customer-service-0.1.30.tgz', client: true
    }))
