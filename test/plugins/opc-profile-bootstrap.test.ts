...
describe('ensureOpcDesktopProfile', () => {
  it('adds only bundled, verified OPC plugins to the initial web profile', async () => {
    ...
    await Promise.all([
      ...
      writeFile(join(plugins, 'nanmicoder-dsh-agent-teams-0.1.8-opc-desktop.3.tgz'), ''),
    ...
    try {
      ...
      expect(manifest.dependencies).toEqual({
        ...
        '@nanmicoder/dsh-agent-teams': `file:${join(plugins, 'nanmicoder-dsh-agent-teams-0.1.8-opc-desktop.3.tgz')}`
      })
