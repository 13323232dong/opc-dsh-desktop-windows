...
describe('desktop WeChat customer service', () => {
  it('ships the second brain retrieval dependency in both profile and bootstrap', () => {
    expect(DESKTOP_PROFILE_MANIFEST.plugins.find(p => p.name === '@opc/dsh-second-brain')).toMatchObject({version:'0.1.6'});
  ...
  it('registers one portable runtime and client with an explicit unsupported Windows adapter', () => {
    ...
    expect(plugins[0]).toMatchObject({ version: '0.1.30', client: true, artifact: 'plugins/opc-DSH-ai-customer-service-0.1.30.tgz' })
  ...
  it('ships the permission tool and a callable AI customer-service tab label', () => {
    const artifact = resolve(
      'packages/opc-profile/plugins/opc-DSH-ai-customer-service-0.1.30.tgz'
    )
    ...
    expect(runtime).toContain("['style_learn'")
    expect(runtime).toContain("['style_delete'")
    expect(runtime).toContain("['analyze'")
    const decodedClient = clientBundle.replace(/\\u([0-9a-f]{4})/gi, (_, hex) => String.fromCharCode(parseInt(hex, 16)))
    expect(decodedClient).toContain('监听范围：全部微信私聊')
    expect(decodedClient).toContain('开始监听')
    expect(decodedClient).not.toContain('管理标签')
    ...
    expect(client).toContain('全部微信私聊')
    expect(client).not.toContain('管理标签')
