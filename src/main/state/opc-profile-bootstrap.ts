...
export const OPC_DESKTOP_PLUGINS = [
  ...
  ['@nanmicoder/dsh-agent-teams', 'nanmicoder-dsh-agent-teams-0.1.8-opc-desktop.9.tgz'],
...
const REMOVED_DESKTOP_PLUGINS = ['@opc/dsh-secure-qr']
...
/**
 * Materializes the single audited desktop baseline after DSH creates its web
 * profile. Every entry is an immutable release artifact; account data stays
 * outside this shared app resource directory.
 */
export async function ensureOpcDesktopProfile(dshHome: string, artifactDirectory: string): Promise<OpcDesktopProfileResult> {
  ...
  let changed = false
  for (const name of REMOVED_DESKTOP_PLUGINS) {
  ...
  const withoutRemovedPlugins = removeRemovedPluginPatches(normalizedPatch)
  const withoutDesktopBaseline = removeDesktopBaseline(withoutRemovedPlugins)
  if (withoutDesktopBaseline !== normalizedPatch || !normalizedPatch.includes(OPC_DESKTOP_PATCH_MARKER)) {
    await writeFile(patchPath, `${withoutDesktopBaseline}${withoutDesktopBaseline ? '\n' : ''}${OPC_DESKTOP_PATCH}`, 'utf8')
    changed = true
...
function removeRemovedPluginPatches(value: string): string {
