...
- 安装验收：从桌面 main `1119ce4` 打包并覆盖 `/Applications/Evan超级管家.app`；全量 903 项测试及品牌、包内资源门禁通过。重启后实时查询为 active 且存在当前问题，真实点击进入全屏“聆听中”，语音 sessionId 与访谈 conversationId 一致。
- 未决：未代填商户资料；界面状态不等于听觉验证。首题实际听感、真实麦克风回答后推进下一题仍待用户现场确认。未公开发布或验证 Windows 安装包。
